...
const text = m.body.slice(prefix.length + cmd.length).trim();
  ...
  if (validCommands.includes(cmd)) {
    ...
    try {
      await m.React('🕘');
      await m.reply('A moment, *LINAH-MD* is generating your lyrics request...');
      ...
      if (result && result.lyrics) {
        ...
        let buttons = [{
          ...
          },
          {
            name: "cta_url",
            buttonParamsJson: JSON.stringify({
              display_text: "sʜᴏᴡ 💜 ғᴏʀ Lɪɴᴀʜ Mᴅ",
              url: `https://www.instagram.com/thedeveloper03/profilecard/?igsh=ejgweHJqNThxNXo3`
            })
          },
          {
            name: "quick_reply",
