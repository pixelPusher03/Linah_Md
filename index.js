...
async function start() {
    try {
        ...
        console.log(`🤖 LINAH-MD using WA v${version.join('.')}, isLatest: ${isLatest}`);
        
        const Matrix = makeWASocket({
            version,
            logger: pino({ level: 'silent' }),
            printQRInTerminal: useQR,
            browser: ["LINAH-MD", "safari", "3.3"],
            auth: state,
            getMessage: async (key) => {
                ...
                return { conversation: "LINAH-MD whatsapp user bot" };
            }
        });
    ...
    try {
        ...
        if (mek.key && mek.key.remoteJid === 'status@broadcast' && config.AUTO_STATUS_SEEN) {
            ...
            if (config.AUTO_STATUS_REPLY) {
                const customMessage = config.STATUS_READ_MSG || '✅ Auto Status Seen Bot By LINAH-MD-V1';
